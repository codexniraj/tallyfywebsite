...
const fetchUploadedFiles = async () => {
  ...
  try {
    ...
    const filesRes = await axios.get("http://localhost:3001/api/getUserSalesUploads", {
      params: { 
    ...
    const filesWithCounts = await Promise.all(
      filesRes.data.map(async (file, index) => {
        try {
          const dataRes = await axios.get("http://localhost:3001/api/getSalesData", {
...
const handleDeleteSelected = async (tableName) => {
  try {
    await axios.delete('http://localhost:3001/api/deleteSalesUpload', {
...
<template>
  <div>
    ...
    <FileUploadDialog
      ...
      upload-api="http://localhost:3001/api/uploadSales"
