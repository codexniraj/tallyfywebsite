...
const compulsoryHeaders = [
  "Name", "Unit", "Group", "Opening Stock", "Rate"
]

const fetchUploadedFiles = async () => {
  try {
    const filesRes = await axios.get("http://localhost:3001/api/getUserItemsUploads", {
    ...
    const filesWithCounts = await Promise.all(
      filesRes.data.map(async (file, index) => {
        try {
          const dataRes = await axios.get("http://localhost:3001/api/getItemsData", {
...
const handleViewUpload = (file) => {
  ...
  sessionStorage.setItem('uploadMeta', JSON.stringify({ invalidItems: file.invalid_items || [] }))
  navigateTo('/items-view')
}

const handleDeleteSelected = async (tableName) => {
  try {
    await axios.delete('http://localhost:3001/api/deleteItemsUpload', {
...
const handleUploadSuccess = (result) => {
  uploadedFiles.value = [
    ...uploadedFiles.value,
    {
      ...
      invalid_items: result.response.invalidItems || [],
      rowCounts: {
        total: 0,
        saved: 0,
        sentToTally: 0
...
<template>
  <div>
    ...
    <FileUploadDialog
      ...
      title="Upload Items Data"
      upload-api="http://localhost:3001/api/uploadItems"
