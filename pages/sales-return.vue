...
const fetchUploadedFiles = async () => {
  try {
    const filesRes = await axios.get("http://localhost:3001/api/getUserSalesReturnUploads", {
    ...
    const filesWithCounts = await Promise.all(
      filesRes.data.map(async (file, index) => {
        try {
          const dataRes = await axios.get("http://localhost:3001/api/getSalesReturnData", {
...
const handleViewUpload = (file) => {
  ...
  navigateTo('/sales-return-view')
}

const handleDeleteSelected = async (tableName) => {
  try {
    await axios.delete('http://localhost:3001/api/deleteSalesReturnUpload', {
...
<template>
  <div>
    ...
    <FileUploadDialog
      ...
      title="Upload Sales Return Data"
      upload-api="http://localhost:3001/api/uploadSalesReturn"
