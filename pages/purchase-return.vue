...
const compulsoryHeaders = [
  "Invoice No", "Date", "Party Name", "Item Name", "Quantity", "Rate", "Amount"
]

const fetchUploadedFiles = async () => {
  try {
    const filesRes = await axios.get("http://localhost:3001/api/getUserPurchaseReturnUploads", {
    ...
    const filesWithCounts = await Promise.all(
      filesRes.data.map(async (file, index) => {
        try {
          const dataRes = await axios.get("http://localhost:3001/api/getPurchaseReturnData", {
...
const handleViewUpload = (file) => {
  ...
  navigateTo('/purchase-return-view')
}

const handleDeleteSelected = async (tableName) => {
  try {
    await axios.delete('http://localhost:3001/api/deletePurchaseReturnUpload', {
...
<template>
  <div>
    ...
    <FileUploadDialog
      ...
      title="Upload Purchase Return Data"
      upload-api="http://localhost:3001/api/uploadPurchaseReturn"
