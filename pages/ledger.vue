...
const fetchUploadedFiles = async () => {
  ...
  try {
    ...
    const filesRes = await axios.get("http://localhost:3001/api/getUserLedgerUploads", {
      params: { 
    ...
    const filesWithCounts = await Promise.all(
      filesRes.data.map(async (file, index) => {
        try {
          const dataRes = await axios.get("http://localhost:3001/api/getLedgerData", {
...
const handleDeleteSelected = async (tableName) => {
  try {
    await axios.delete('http://localhost:3001/api/deleteLedgerUpload', {
...
<template>
  <div>
    ...
    <FileUploadDialog
      ...
      upload-api="http://localhost:3001/api/uploadLedger"
